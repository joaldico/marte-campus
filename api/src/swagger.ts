...
export function setupSwagger(app: INestApplication): void {
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'OpenAPI generated from Nest decorators (health, auth, catalog, player chapter, video stream, playback events and cursor, admin course CRUD, status transition, and revision submit/publish).',
    )
    .setVersion('1.0')
